...
/*
  Cette classe permet de créer des hashs sécurisés de mot de passe et de les vérifier.

  La fonction createHash($password) permet de générer le hash
  La fonction verify_password($password, $hash) permet de vérifier si un mot de passe correspond à un hash

  La classe gère elle même l'ajout d'un salt

  Les algorithmes utilisés ne sont pas "memory hard", ils n'ont besoin que de ressources CPU et très peu de ressources mémoire.
  On pourra à terme changer d'algorithme (voir scrypt / libsodium) pour rendre plus robustes les mots de passe.
*/
